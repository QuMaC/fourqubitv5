    ...
    with program() as qubit_spec:
        ...
        with for_(n, 0, n < avgs, n + 1):
            with for_(f, f_min, f < f_max, f + df):
                ...
                reset_frame(qe12)
                # wait(20000, qe)
                cooldown(time=20000, active_reset=False, qe=qe)
                play_X180(qe)
                align(qe, qe12)
                update_frequency(qe12, f)
                play("const" * amp(q_amp), qe12, duration=200)
                align(rr, qe12)
...
######################################
f_min, f_max = -1*0.8*(q_IF[str(q_no)] + 150e6)/1e6, 0.8*(q_IF[str(q_no)] - 150e6)/1e6  # in MHz
...
with open('../Configuration_Files/System_Parameters/external_bandwidth.json','r') as f:
    ...
    f.close()
if q_no < 5:
    power_scale = abs(np.round((4.9296223/ext_bw[f'{q_no}']), 5) * 1)

else:
    power_scale = abs(np.round((3.9296223 / ext_bw[f'{q_no}']), 5) * 1)
...
amp_range = [a_min*power_scale, min(min(a_max, 0.8)*power_scale, 1.0)]
amp_range = [0.7*a_min*power_scale, 2*a_min*power_scale]
check_by2_line = 0
    ...
    if init_flag == 1:
        ...
        for data_i in data:
            if dat_index is not None:
                ...
                normed = np.roll(normalize(fltd[w_size:-w_size + 1]), w_size // 2)
                if check_by2_line == 1:
