"""Parallel nsub4 flat @ ZZ=300 only — isolated results folder.

Matches the live ``cr_grape_robust_zz_sweep_resume_nsub4.py`` flat knobs
(n_sub=4, maxiter=180, mean_minus_spread, …) but:

- runs **only** flat ZZ = 300 kHz
- writes to ``results/robust_zz_sweep_nsub4_flat300_parallel/``
  (not ``robust_zz_sweep_nsub4``), so it cannot clash with the live flat
  job that is still on 250 → would otherwise continue into 300 there

Safe to run in a separate tmux while ``cz_grape_flat`` is busy.

After this finishes you can compare / copy artifacts into the main tree
if you want a single folder layout.
"""
...
# Isolated top folder — do not point this at robust_zz_sweep_nsub4.
RESULTS_NAME = "robust_zz_sweep_nsub4_flat300_parallel"
RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results", RESULTS_NAME)
os.makedirs(RESULTS_DIR, exist_ok=True)

# ---------------------------------------------------------------------------
# Knobs — same as nsub4 flat resume
# ---------------------------------------------------------------------------
...
ZZ_SPANS_KHZ = [300]
RESUME_ZZ_SPANS_KHZ = [300]

SEED_MODES: Literal["flat", "warm", "both"] = "flat"
...
def _load_existing_runs(results_dir: str) -> list[dict]:
...
def _write_summary(results_dir: str, rows: list[dict]) -> None:
    ...
    payload = {
        ...
        "resume_zz_spans_khz": list(RESUME_ZZ_SPANS_KHZ),
        "config": {
            ...
            "resume_script": os.path.basename(__file__),
            "note": (
                "Parallel flat-300 only; isolated from robust_zz_sweep_nsub4 "
                "while that job finishes 250."
            ),
        },
        "runs": rows,
    }
...
def run_robust_zz_sweep() -> list[dict]:
    """Run nsub4-equivalent flat ZZ=300 into an isolated results folder."""
    ...
    rows = _load_existing_runs(RESULTS_DIR)
    done = {(int(r["zz_span_khz"]), str(r["mode"])) for r in rows}
    print(f"\nIsolated run: results={RESULTS_NAME}")
    print(f"Resume: loaded {len(rows)} existing run(s) from summary")
    if done:
        print(f"  already done: {sorted(done)}")
    print(f"spans: {RESUME_ZZ_SPANS_KHZ}  modes={modes}")
    print(
        f"SEED_MODES={SEED_MODES} -> branches {modes}  |  "
        f"amp={CR_PULSE_PARAMS['amp_mhz']} MHz  flat={FLAT_LEN_NS:.0f} ns  "
        f"knobs={N_FLAT_KNOBS}  n_sub={N_SUB}  maxiter={MAXITER}"
    )

    for zz_khz in RESUME_ZZ_SPANS_KHZ:
        zz_mhz = float(zz_khz) / 1000.0
        for mode in modes:
            out_dir = _span_dir(RESULTS_DIR, mode, zz_khz)
            os.makedirs(out_dir, exist_ok=True)

            if (zz_khz, mode) in done:
                print(f"\nSKIP zz={zz_khz} kHz mode={mode} (already in summary)")
                continue
            ...
            optimizer = RobustCRGrapeOptimizer(config, flat_knobs_seed=None)
