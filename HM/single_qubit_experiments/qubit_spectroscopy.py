...
class QubitSpectroscopy(SingleQubitExperiment):
    ...
    def _identify_qubit_peaks(
        ...
        for q_idx, val in enumerate(p_arr[0]):
            ...
            # threshold: must appear in at least 2 other power levels
            # (det_count check below is implicit via cross-validation indices)
    ...
    def _zoom_sweep(self, f_center_MHz: float, best_quad: int) -> float:
        ...
        while True:
            ...
            f_center_hz = float(freqs[best_idx + w // 2])
    ...
    def run_experiment(self):
        ...
        if len(qubits) > 1:
            logger.warning(f"Multiple candidates: {np.round(qubits, 3)} MHz — using lowest")

        f_center_MHz      = float(np.min(qubits))
