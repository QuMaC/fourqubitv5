    ...
    with for_(n, 0, n < 200, n + 1):
        with for_(f, f_min, f < f_max, f + df):
            ...
            if simulate:
                ...
                cooldown(time=rep_rate_clk- ro_len)
            play("I", qe)
            align(qe, rr)
            wait(4, rr)
            measure_macro(qe, rr, out, I0, Q0, pi_12=Pi12)
...
if simulate:
    ...
    control_I = getattr(samples, con).analog[f'{dac_mapping[qe][1][0]}']
    control_Q = getattr(samples, con).analog[f'{dac_mapping[qe][1][1]}']
    rd_t_I = getattr(samples, con).analog[f'{dac_mapping[rr][1][0]}']
    rd_t_Q = getattr(samples, con).analog[f'{dac_mapping[rr][1][1]}']
