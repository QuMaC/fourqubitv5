...
def TwoQ_RB_fixed_seq(op_list, cr_elem, qe_c, qe_t, AC_flag = False):
    def play_seq(op_list, cr_elem, qe_c, qe_t, AC_flag = False):
        ...
        for op in op_list:
            ...
            if op[0] == "CNOT":
                ...
                # ZXby4_AC(cr_elem, "cr_ac_c3t4", qe_c, a_cr=cr_amp[cr_elem], a_ac=cr_amp["cr_ac_c3t4"],
                ...
                align(cr_elem, qe_c, qe_t)
                ZXby4(cr_elem, qe_c, a=-cr_amp[cr_elem], t=cr_len_ns[cr_elem] // 4)
                # ZXby4_AC(cr_elem, "cr_ac_c3t4", qe_c, a_cr= -cr_amp[cr_elem], a_ac= -cr_amp["cr_ac_c3t4"], t=cr_len_ns[cr_elem] // 4)
    ...
    with program() as twoQ_RB:
        ...
        with for_(n, 0, n < n_avg, n + 1):
            wait(wait_init, qe_c)
            align(qe_c, rr_c, rr_t)
            measure_macro(qe_c, rr_c, out_c, I0c, Q0c, pi_12=pi_12)
            save(I0c, I0c_st)
            save(Q0c, Q0c_st)

            measure_macro(qe_c, rr_t, out_t, I0t, Q0t, pi_12=pi_12)
            save(I0t, I0t_st)
            save(Q0t, Q0t_st)

            align(rr_c, qe_c, qe_t)
            ...
            align(rr_c, qe_t)
            wait(wait_init, qe_t)
            align(qe_t, qe_c, cr_elem)
            play_seq(op_list, cr_elem, qe_c, qe_t)
...
for exp, exp_in in zip(exp_seq, exp_in_seq):
    ...
    It, Qt, Ic, Qc, I0c, I1c, I0t, I1t = TwoQ_RB_fixed_seq(op_list, cr_elem, qe_c, qe_t, AC_flag)
...
y1ini = np.array([y1_in_data[i][0] for i in range(n_seq * n_el)])
y2ini = np.array([y2_in_data[i][0] for i in range(n_seq * n_el)])

y1i_reshaped = y1i.reshape((n_seq, n_el))
y2i_reshaped = y2i.reshape((n_seq, n_el))
y1ini_reshaped = y1ini.reshape((n_seq, n_el))
y2ini_reshaped = y2ini.reshape((n_seq, n_el))
...
y1ini = [y1_in_data[i][0] for i in sort_i]
y2ini = [y2_in_data[i][0] for i in sort_i]
...
print("=====================INTERLEAVED===================================================================")
pars1_in = fit_RB(x_fit, y1ini_fit, nqubits=2, ivals=[0.5, 0, 0.9])

print("========================Readout : Control Qubit====================================================")
print("=====================STANDARD======================================================================")
pars2 = fit_RB(x_fit, y2i_fit, nqubits=2, ivals=[-0.5, 0, 0.8])
print("=====================INTERLEAVED===================================================================")
pars2_in = fit_RB(x_fit, y2ini_fit, nqubits=2, ivals=[-0.5, 0, 0.8])
...
plt.plot(x_fit, power_law(x_fit, *pars1_in), '-b')
plt.title(f"2Q RB: Readout Target  F(CNOT) = {ZX_fid1}%")
...
plt.plot(x_fit, power_law(x_fit, *pars2_in), '-b')
plt.title(f"2Q RB: Readout Control  F(CNOT) = {ZX_fid2}%")
plt.ylabel("Voltage (a.u.)")
plt.xlabel("No. of Cliffords")
...
if save_data is True:
    ...
    file_saver_qubit_(np.transpose([x_data_sorted, y1ini]), file_name=__file__, master_folder=ExpName,
    ...
    file_saver_qubit_(np.transpose([x_fit, power_law(x_fit, *pars1_in)]), file_name=__file__, master_folder=ExpName,
    ...
    file_saver_qubit_(np.transpose([x_data_sorted, y2ini]), file_name=__file__, master_folder=ExpName,
    ...
    file_saver_qubit_(np.transpose([x_fit, power_law(x_fit, *pars2_in)]), file_name=__file__, master_folder=ExpName,
