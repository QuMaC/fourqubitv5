...
a_cr = amp_scale[str(c_no)]["X180"]*1.5
a_cr = cr_amp[cr_elem]
...
# for p in p_list:
while abs(str_phase) > 0.005 and trial_cnt < max_trials:

    job = HT_setPhase_local_phase(qmm, cr_elem, p, t_min, t_max, dt, n_avg, wait_init, wait_t,
    ...
    fig.suptitle(f"Cross Resonance Tomography : Phase {p}", fontsize=15)
    ...
    print(f'Strengths at {acc_phase} are '
    ...
    print(f'Current phase offset = {acc_phase}')
    p = acc_phase
    trial_cnt += 1

print(f'Optimal CR phase is {acc_phase}')
print(f'Strengths at {acc_phase} are '
      f'\n ZX = {int_strengths[0] * 1e3} MHz'
      f'\n IX = {int_strengths[1] * 1e3} MHz'
      f'\n ZY = {int_strengths[2] * 1e3} MHz'
...
if update_calib:
    ...
    cr_phase_data[cr_elem] = acc_phase
