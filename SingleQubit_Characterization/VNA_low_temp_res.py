...
# for q_no in [3]:
    ...
    def ext_BW(freq, ydata, plot=False):
        ...
        if plot:
            lorentzian_fit = lorentzian(freq, res[0], res[1], res[2], res[3])
            freq_ghz = freq * 1e-9
            # save the data
            txt_path = Path.cwd() / f'Cavity_response_Real_for_Cavity_{(2*q_no+5)%12}.txt'
            data_to_save = np.column_stack((freq_ghz, ydata, lorentzian_fit))
            np.savetxt(txt_path, data_to_save, delimiter='\t', header='Frequency (GHz)\tData\tFit')
            print(f"Saved cavity response data to: {txt_path.as_uri()}")
            # plt.figure()
            fig, ax = plt.subplots()
            ax.plot(freq_ghz, ydata, label="Data")
            ax.plot(freq_ghz, lorentzian_fit, c='r', label="Fit")
    ...
    for f in f_i:
        kna.write(f"SENS1:FREQ:START {f}")
        time.sleep(cmd_delay)
        kna.write("SENS1:FREQ:STOP 7.75e9")
        time.sleep(cmd_delay)
    ...
    # data3, cov3 = np.polyfit(f_data[len_f // 2:], ph_data[len_f // 2:], 1, cov=True)
    ...
    kna.write(f"CALC:MEAS:OFFS:PHAS {ph_offset}")
    ph_data = np.array(kna.query_ascii_values("CALC1:MEAS1:DATA:FDAT?"))
    time.sleep(cmd_delay)
    f_data = np.array(kna.query_ascii_values("CALC1:MEAS1:X:VAL?"))
    time.sleep(cmd_delay)
    ...
    ext_bw[f'{q_no}'] = np.round(meas_vals_l[2][0], 5)

    with open('../Configuration_Files/System_Parameters/external_bandwidth.json', 'w') as f:
        json.dump(ext_bw, f, indent=6)
        f.close()
    ...
    int_bw[f'{q_no}'] = np.round(meas_vals_l[1][0], 5)

    with open('../Configuration_Files/System_Parameters/internal_bandwidth.json', 'w') as f:
        json.dump(int_bw, f, indent=6)
        f.close()
    ...
    if manual_punch_out == 1:
        ...
        path1 = path + ExpName + r"\Low Temperature Response" + test_name
        fname = path + ExpName + r"\Low Temperature Response" + test_name + f'{high_power}_dB.png'
        plt.savefig(fname=fname, bbox_inches='tight')
        # Print as file URI so Ctrl+Click works even with spaces in the path
        print(f"Saved high-power cavity response figure to: {Path(fname).as_uri()}")
    ...
    if update_config_dicts == 1:
        try:
            # Get the correct DAC key for the qubit
            dac_key = keyer(f'q{q_no}', dac_mapping)[0]

            # Open the RF source connection
            rr_LO_rns = rm.open_resource(LO_IP_dict['rr_LO'][dac_key])

            # Use the low-power fit result for the resonator frequency
            resonator_freq = meas_vals_l[0][0]  # Resonator frequency in GHz (from ext_BW)
            lo_freq = resonator_freq - 0.02  # Set LO frequency 20 MHz below resonator

            # Set the RF source frequency
            rr_LO_rns.write(f'SOUR:FREQ:CW {lo_freq} GHz')

            # Load existing configuration dictionaries
            try:
            ...
            rr_LO_dict[str(q_no)] = lo_freq
            rr_IF_dict[str(q_no)] = 20  # 20 MHz IF frequency

            # Create directory if it doesn't exist
            os.makedirs('./System_Parameters', exist_ok=True)
            ...
            # Close the RF source connection
            rr_LO_rns.close()

            print(f"Updated configuration for qubit {q_no}: LO = {lo_freq:.6f} GHz, IF = 20 MHz")

        except Exception as e:
            print(f"Error updating configuration dictionaries: {e}")
            if 'rr_LO_rns' in locals():
                rr_LO_rns.close()
