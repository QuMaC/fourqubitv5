...
t = np.arange(0,4*ro_len,1)

rr_IF_LO_a = np.sin(2*np.pi*X[a]*t*1e-9)

sos = sp.signal.butter(20, X[a], btype = 'low', analog = False, output = 'sos', fs = 1e9)

bb_ch1 = rr_IF_LO_a * adc1
bb_ch2 = rr_IF_LO_a * adc2

fltd1 = sp.signal.sosfilt(sos, bb_ch1)
fltd2 = sp.signal.sosfilt(sos, bb_ch2)

plt.figure()
plt.plot(t*1e-9, bb_ch1)
plt.plot(t*1e-9, fltd1)
plt.grid()

# plt.figure()
# plt.plot(t*1e-9, bb_ch2)
# plt.plot(t*1e-9, fltd2)
# plt.grid()