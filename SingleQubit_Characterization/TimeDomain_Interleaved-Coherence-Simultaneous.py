...
plot_final = False  # Set as true to obtain final plots with data and fit lines
...
dt = 100 // 4  # actual step size is twice this to keep ramsey and t1 steps consistent with echo #150//4
#changed above from 200//4 to 100//4
...
for i in range(n_runs):
    ...
    def coh_analysis(Ir, Ie, It, Qr, Qe, Qt, q_no, k, verbose=True, plot_final=True, save_data=False):
        ...
        if verbose:
            ...
            print('#########################')
            print('Ramsey Freq = {0}'.format(ram_f))
